...
class Zfs_Refcount(sdb.Locator, sdb.PrettyPrinter):
    ...
    @staticmethod
    def print_ref(obj: drgn.Object, indent: int) -> None:
        ptr = int(obj.ref_holder)
        c = sdb.create_object("char *", ptr)
        s = c.string_().decode("utf-8")
        print(f"{' '*indent}{hex(ptr)}   {s} ")

    @staticmethod
    def print_one(zr: drgn.Object, indent: int) -> None:
        ...
        try:
            ...
            pass
        print(
            f"{' '*indent}zfs_refcount_t at {hex(int(zr))} has {int(zr.rc_count)} "
        ...
        if tracked:
            ...
            for ref in refs:
                Zfs_Refcount.print_ref(ref, indent * 2)
